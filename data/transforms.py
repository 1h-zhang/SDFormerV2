...
class PadImage:
    ...
    def __init__(self, size):
        ...
        self.fill_index = {'edge': 255,
                           'human_parts': 255,
                           'semseg': 255,
                           'depth': 0,              # 对于cityscape无效值为-1，nyudv2无效值为0
                           'normals': [0, 0, 0],
                           'sal': 255,
                           'image': [0, 0, 0]}
...
class RandomCrop:
    ...
    def random_crop(self, key, uncropped, crop_loc):
        ...
        cropped = uncropped[crop_loc[0]:crop_loc[1],
                            crop_loc[2]:crop_loc[3], :]     # 取的长度超过了原图尺寸，那么就等于原图尺寸
        return cropped
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            ...
            if elem == 'normals':
                ...
                # tmp[tmp < 0.] = -1
                # sample[elem] = tmp            # 对于cityscape当小于0时，使其为-1  适用于Cityscapes
