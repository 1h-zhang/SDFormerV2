...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, out, label, reduction='mean'):
        label = torch.squeeze(label, dim=1).long()      # long() == int64
        if self.balanced:
...
class L1Loss(nn.Module):
    ...
    def forward(self, out, label, reduction='mean'):
        ...
        n_valid = torch.sum(mask).item()
        masked_out = torch.masked_select(out, mask)     # masked_select输出一个一维向量，mask.shape不一定要和out匹配但要可广播
        masked_label = torch.masked_select(label, mask)
        if reduction == 'mean':
