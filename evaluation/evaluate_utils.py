...
def get_single_task_meter(p, database, task):
    ...
    if task == 'semseg':
        ...
        from evaluation.eval_depth import DepthMeter, DepthMeter_v1
        # return DepthMeter(ignore_index=p.ignore_index)
        return DepthMeter_v1(max_depth=p.TASKS.depth_max, min_depth=p.TASKS.depth_min)
...
def save_model_pred_for_one_task(p, sample, output, save_dirs, task=None, epoch=None):
    ...
    for jj in range(int(inputs.size()[0])):
        ...
        im_height = meta['img_size'][jj][0]     # 一个batch中取出某张图的高宽
